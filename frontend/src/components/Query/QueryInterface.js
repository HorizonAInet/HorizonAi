...
export const QueryInterface = ({
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-white dark:bg-github-dark border border-gray-200 dark:border-black rounded-lg shadow-sm p-4">
        <button
        ...
        >
          <div className="flex items-center space-x-2">
            ...
            <h3 className="text-gray-800 dark:text-white font-bold">
              Sample Queries
            </h3>
          </div>
          {showSampleQueries ? (
            <ChevronUp className="h-5 w-5 text-gray-500 dark:text-gray-200" />
          ) : (
            <ChevronDown className="h-5 w-5 text-gray-500 dark:text-gray-200" />
          )}
        </button>
          ...
          <div className="mt-3 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-2">
            {sampleQueries.map((sampleQuery, index) => (
              <button
                ...
                className="text-left p-2 bg-gray-100 dark:bg-github-dark hover:bg-gray-200 dark:hover:bg-black rounded text-sm text-gray-700 dark:text-gray-200 transition-colors"
              >
                {sampleQuery}
              </button>
            ))}
        ...
        <div className="bg-white dark:bg-github-dark border border-gray-200 dark:border-black rounded-lg shadow-sm p-4">
          ...
          <div className="flex space-x-2">
            <textarea
              ...
              className="flex-1 min-h-[60px] max-h-[200px] p-3 bg-gray-50 dark:bg-black border border-gray-300 dark:border-black rounded-lg text-gray-800 dark:text-white placeholder-gray-400 dark:placeholder-gray-300 focus:outline-none focus:ring-0 focus:ring-logocolor resize-none font-normaltext"
            ...
            <div className="flex flex-col space-y-2">
              <button
                ...
                className="bg-black dark:bg-github-dark text-white dark:text-white p-3 rounded-lg focus:outline-none focus:ring-0 focus:ring-logocolor disabled:opacity-50 disabled:cursor-not-allowed transition-colors hover:bg-gray-800 dark:hover:bg-black"
              >
                {isProcessing ? (
      ...
      {error && (
        <>
          {isAPIKeyError ? (
            ...
            <div className="bg-white dark:bg-github-dark border border-red-500 dark:border-red-400 rounded-lg p-4">
              <div className="flex items-center space-x-2">
                <svg
                ...
                <p className="text-gray-800 dark:text-gray-200 font-medium text-sm">{error}</p>
              </div>
      ...
      {sessionResults.length > 0 && (
        <div className="space-y-6">
          <div className="flex items-center justify-between">
            <h3 className="text-xl font-jersey text-gray-800 dark:text-gray-200">
              Query Results ({sessionResults.length} total)
            </h3>
            <button
              onClick={() => setShowGeneratedCode(!showGeneratedCode)}
              className="flex items-center space-x-2 bg-gray-100 dark:bg-github-dark text-gray-700 dark:text-gray-200 px-4 py-2 rounded-lg hover:bg-gray-200 dark:hover:bg-black transition-colors font-normaltext"
            >
          ...
          {visibleResults.map((result, index) => (
            <div
              key={index}
              className="bg-white dark:bg-github-dark border border-gray-200 dark:border-black rounded-lg shadow-sm overflow-hidden"
            >
              <div className="bg-gray-100 dark:bg-black px-4 py-3 border-b border-gray-200 dark:border-black">
                <div className="flex items-center justify-between">
                  <h4 className="text-gray-800 dark:text-gray-200 font-normaltext font-medium">
                    Query: {result.query}
                  </h4>
              ...
              <div className="p-4">
                ...
                {result.result?.type === "dataframe" && result.result.data && (
                  <div className="overflow-x-auto">
                    <table className="w-full text-sm border border-gray-200 dark:border-black rounded-lg">
                      <thead className="bg-gray-50 dark:bg-black">
                        <tr className="border-b border-gray-200 dark:border-black">
                          {result.result.columns?.map((column, colIndex) => (
                            <th
                              key={colIndex}
                              className="text-left py-2 px-3 text-gray-700 dark:text-gray-300 font-normaltext font-semibold"
                            >
                              {column}
                            </th>
                          ))}
                      ...
                      <tbody>
                        {result.result.data.map((row, rowIndex) => (
                          <tr
                            key={rowIndex}
                            className="border-b border-gray-100 dark:border-black"
                          >
                            {row.map((cell, cellIndex) => (
                              <td
                                key={cellIndex}
                                className="py-2 px-3 text-gray-600 dark:text-gray-400 font-normaltext"
                              >
                                {cell}
                              </td>
                            ))}
                    ...
                    {result.result.shape && (
                      <p className="text-xs text-gray-500 dark:text-gray-200 mt-2 text-center">
