...
const FeedbackModal = ({ isOpen, onClose, onSubmit }) => {
  ...
  const renderRatingScale = (field, label, description) => {
    ...
    return (
      <div className="mb-6">
        <h3 className="text-sm font-medium text-gray-800 dark:text-gray-200 mb-1">{label}</h3>
              ...
              className={`w-10 h-10 rounded-md border text-sm font-semibold flex items-center justify-center transition-all duration-200
                ${
                  currentRating === rating
                    ...
                    : "border-gray-300 dark:border-black text-gray-500 dark:text-gray-400 hover:border-yellow-400 hover:text-yellow-500 dark:hover:border-yellow-500 dark:hover:text-yellow-400"
                }`}
            >
              {rating}
            </button>
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-30 dark:bg-black dark:bg-opacity-50 flex items-center justify-center z-50 p-4">
      <div className="bg-white dark:bg-github-dark rounded-lg border border-gray-200 dark:border-black w-full max-w-md shadow-lg max-h-[90vh] overflow-y-auto">
        {/* Header */}
        <div className="flex items-center justify-between p-5 border-b border-gray-200 dark:border-black">
          <div className="flex items-center space-x-2">
            <MessageSquare className="text-yellow-500 dark:text-yellow-400" size={22} />
        ...
        <form onSubmit={handleSubmit} className="p-5">
          ...
          <div className="mb-6">
            ...
            <textarea
              ...
              placeholder="Tell us more about your experience..."
              className="w-full px-3 py-2 border border-gray-300 dark:border-black bg-white dark:bg-github-dark rounded-md text-gray-700 dark:text-gray-100 placeholder-gray-400 dark:placeholder-gray-500 focus:outline-none focus:ring-2 focus:ring-yellow-400 focus:border-yellow-400 resize-none"
              rows={4}
            />
          </div>
          ...
          <div className="flex space-x-3">
            <button
              type="button"
              onClick={onClose}
              className="flex-1 px-4 py-2 border border-gray-300 dark:border-black text-gray-600 dark:text-gray-300 bg-white dark:bg-github-dark rounded-md hover:bg-gray-100 dark:hover:bg-black transition-colors"
            >
              Cancel
            </button>
            <button
              type="submit"
              disabled={isSubmitting}
              className="flex-1 px-4 py-2 bg-yellow-400 dark:bg-yellow-500 text-black rounded-md hover:bg-yellow-500 dark:hover:bg-yellow-600 transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
            >
              {isSubmitting
                ? "Submitting..."
                : existingFeedback
