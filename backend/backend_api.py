...
async def upload_file(
    ...
    try:
        ...
        try:
            ...
            # Generate file ID
            file_id = f"{user_id}_{datetime.now().strftime('%Y%m%d_%H%M%S')}_{file.filename}"
...
async def process_query(
    ...
    try:
        ...
        try:
            ...
            print(f"Fuzzy matching failed, falling back to basic processing: {fuzzy_error}")
        ...
        if not result['success'] and result.get('error'):
            ...
            if "RATE_LIMIT_EXCEEDED" in error_msg or "rate limit" in error_msg.lower() or "too many requests" in error_msg.lower():
        ...
        if result['success'] and result['result'] is not None:
            if isinstance(result['result'], pd.DataFrame):
                ...
                processed_result = {
                    ...
                    'data': result['result'].head(100).values.tolist(),  # Limit to 100 rows
...
async def submit_feedback(
    ...
    try:
        ...
        print(f"Feedback data: accuracy={feedback.accuracy_rating}, speed={feedback.speed_rating}, overall={feedback.overall_rating}")
        
        # Check if user already has feedback
        existing_feedback = supabase_logger.client.table('feedback').select('*').eq('user_id', feedback.user_id).execute()
        ...
        if existing_feedback.data:
            ...
            result = supabase_logger.client.table('feedback').update(feedback_data).eq('user_id', feedback.user_id).execute()
            ...
            result = supabase_logger.client.table('feedback').insert(feedback_data).execute()
            print(f"Insert result: {result}")
            feedback_id = result.data[0]['id'] if result.data else None
            message = "Feedback submitted successfully"
        
        print(f"Final feedback response: success=True, message={message}, feedback_id={feedback_id}")
        return FeedbackResponse(
            success=True,
            message=message,
...
async def get_user_feedback(
    ...
    try:
        ...
        result = supabase_logger.client.table('feedback').select('*').eq('user_id', user_id).execute()
